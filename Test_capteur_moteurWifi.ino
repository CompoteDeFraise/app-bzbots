 ...
 * * + AJOUT : Création du réseau Wi-Fi (Point d'Accès)
 */

#include <WiFi.h> // 🔹 Ajout de la librairie Wi-Fi

// --- Paramètres Wi-Fi (Point d'Accès) ---
const char* ssid = "Machine_UV_Relining";
const char* password = "SuperPassword123"; // 8 caractères minimum !
...
void setup() {
  ...
  // 🔹 --- DÉBUT DE L'AJOUT WI-FI ---
  Serial.println("\nAllumage de l'antenne Wi-Fi...");
  WiFi.softAP(ssid, password);
  Serial.println("Réseau Wi-Fi en ligne !");
  Serial.print("Pour controler la machine, tapez cette IP : ");
  Serial.println(WiFi.softAPIP());
  // 🔹 --- FIN DE L'AJOUT WI-FI ---
  ...
  Serial.println("=== SYSTEME PRET (Version robuste + Wi-Fi) ===");
  faireTournerMoteur();
}
